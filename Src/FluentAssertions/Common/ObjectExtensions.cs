...
namespace FluentAssertions.Common
{
    internal static class ObjectExtensions
    {
        ...
        private static bool CompareNumerics(object actual, object expected)
        {
            ...
            return actualType != expectedType && actual.IsNumericType() && expected.IsNumericType() &&
                CanConvert(actual, expected, actualType, expectedType) && CanConvert(expected, actual, expectedType, actualType);
        }
        ...
        private static bool CanConvert(object source, object target, Type sourceType, Type targetType)
        {
            try
            {
                object converted = source.ConvertTo(targetType);

                return source.Equals(converted.ConvertTo(sourceType)) && converted.Equals(target);
            }
            catch
            {
                // ignored
        ...
        private static bool IsNumericType(this object obj)
        {
            ...
            return obj is not null
                and (int or long or float or double or decimal or sbyte or byte or short or ushort or uint or ulong);
        }
    }
}
