...
public class StringAsyncEnumerableAssertions : StringAsyncEnumerableAssertions<IAsyncEnumerable<string>>
{
    /// <summary>
    /// Initializes a new instance of the <see cref="StringAsyncEnumerableAssertions"/> class.
    /// </summary>
    public StringAsyncEnumerableAssertions(IAsyncEnumerable<string> actualValue)
        : base(actualValue)
    {
    }
}

public class StringAsyncEnumerableAssertions<TCollection>
    : StringAsyncEnumerableAssertions<TCollection, StringAsyncEnumerableAssertions<TCollection>>
    where TCollection : IAsyncEnumerable<string>
{
    /// <summary>
    /// Initializes a new instance of the <see cref="StringAsyncEnumerableAssertions{TCollection}"/> class.
    /// </summary>
    public StringAsyncEnumerableAssertions(TCollection actualValue)
        : base(actualValue)
    {
    }
}

public class StringAsyncEnumerableAssertions<TCollection, TAssertions> : AsyncEnumerableAssertions<TCollection, string, TAssertions>
    where TCollection : IAsyncEnumerable<string>
    where TAssertions : StringAsyncEnumerableAssertions<TCollection, TAssertions>
{
    ...
    public new AndConstraint<TAssertions> Equal(IAsyncEnumerable<string> expected)
    {
        return base.Equal(expected);
    }
    ...
    /// <remarks>
    /// The two collections are equivalent when they both contain the same strings in any order.  To assert that the elements
    /// are in the same order, use <see cref="Equal(IAsyncEnumerable{string})"/> instead.
    /// </remarks>
        ...
        params object[] becauseArgs)
    {
        ...
        string[] repeatedExpectation = RepeatAsManyAs(expectation, BlockingSubject).ToArray();
    ...
    private bool ContainsMatch(string wildcardPattern)
    {
        ...
        return BlockingSubject.Any(item =>
        {
    ...
    {
        return BlockingSubject.Where(item =>
        {
            using var scope = new AssertionScope();
            item.Should().Match(wildcardPattern);
            return scope.Discard().Length == 0;
    ...
    private bool NotContainsMatch(string wildcardPattern)
    {
        ...
        return BlockingSubject.All(item =>
        {
