...
public class TypeAssertions : ReferenceTypeAssertions<Type, TypeAssertions>
{
        ...
        [StringSyntax("CompositeFormat")] string because = "", params object[] becauseArgs)
    {
        ...
        if (assertionChain.Succeeded)
        {
            ...
            var subjectDescription = assertionChain.HasOverriddenCallerIdentifier ? assertionChain.CallerIdentifier : Subject!.Name;

            assertionChain
                .BecauseOf(because, becauseArgs)
                .ForCondition(propertyInfo is not null)
                .FailWith($"Expected {subjectDescription} to have a property {name} of type {propertyType.Name}{{reason}}, but it does not.")
                .Then
                .ForCondition(propertyInfo.PropertyType == propertyType)
                .FailWith($"Expected property {propertyInfo.Name} to be of type {propertyType}{{reason}}, but it is not.", propertyInfo);
        }
        ...
        [StringSyntax("CompositeFormat")] string because = "", params object[] becauseArgs)
    {
        ...
        if (assertionChain.Succeeded)
        {
            ...
            assertionChain
                ...
                .FailWith($"Did not expect {subjectDescription} to have a property {propertyInfo?.Name}{{reason}}, but it does.");
        }
