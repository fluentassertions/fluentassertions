                ...
                else
                {
                    ...
                    if (dataSetConfig?.AllowMismatchedTypes != true && dataTableConfig?.AllowMismatchedTypes != true)
                    {
                        AssertionScope.Current.ForCondition(subject.GetType() == expectation.GetType())
                            .FailWith("Expected {context:DataTable} to be of type '{0}'{reason}, but found '{1}'",
            ...
            Dictionary<string, IMember> selectedMembers)
        {
            ...
            if (selectedMembers.ContainsKey(nameof(expectation.CaseSensitive)))
            {
                AssertionScope.Current.ForCondition(subject.CaseSensitive == expectation.CaseSensitive)
                    .FailWith(
                        "Expected {context:DataTable} to have CaseSensitive value of '{0}'{reason}, but found '{1}' instead",
                        expectation.CaseSensitive, subject.CaseSensitive);
            }

            if (selectedMembers.ContainsKey(nameof(expectation.DisplayExpression)))
            {
                AssertionScope.Current.ForCondition(subject.DisplayExpression == expectation.DisplayExpression)
                    .FailWith(
                        "Expected {context:DataTable} to have DisplayExpression value of '{0}'{reason}, but found '{1}' instead",
            ...
            if (selectedMembers.ContainsKey(nameof(expectation.RemotingFormat)))
            {
                AssertionScope.Current.ForCondition(subject.RemotingFormat == expectation.RemotingFormat)
                    .FailWith(
                        "Expected {context:DataTable} to have RemotingFormat value of '{0}'{reason}, but found '{1}' instead",
