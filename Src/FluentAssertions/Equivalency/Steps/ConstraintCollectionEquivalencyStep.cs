            ...
            IEquivalencyValidator nestedValidator)
        {
            if (comparands.Subject is not ConstraintCollection)
            {
                AssertionScope.Current.FailWith(
                    "Expected a value of type ConstraintCollection at {context:Constraints}, but found {0}",
            ...
            else
            {
                ...
                var subjectConstraints = subject.Cast<Constraint>()
                    .ToDictionary(constraint => constraint.ConstraintName);

                var expectationConstraints = expectation.Cast<Constraint>()
                    .ToDictionary(constraint => constraint.ConstraintName);
                ...
                foreach (string constraintName in constraintNames)
                {
                    AssertionScope.Current
                        ...
                        .FailWith(
                            "Expected constraint named {0} in {context:Constraints collection}{reason}, but did not find one",
                            constraintName);
