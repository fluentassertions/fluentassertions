...
internal class EnumerableEquivalencyValidator
{
        ...
        if (AssertIsNotNull(expectation, subject) && AssertCollectionsHaveSameCount(subject, expectation))
        {
            if (Recursive)
            {
                using var _ = context.Tracer.WriteBlock(member =>
                    Invariant($"Structurally comparing {subject} and expectation {expectation} at {member.Subject}"));
            ...
            else
            {
                using var _ = context.Tracer.WriteBlock(member =>
                    Invariant(
                        $"Comparing subject {subject} and expectation {expectation} at {member.Subject} using simple value equality"));
    ...
    private void AssertElementGraphEquivalencyWithStrictOrdering<T>(object[] subjects, T[] expectations)
    {
        ...
        foreach (int index in Enumerable.Range(0, expectations.Length))
        {
            ...
            using var _ = context.Tracer.WriteBlock(member =>
                Invariant(
                    $"Strictly comparing expectation {expectation} at {member.Subject} to item with index {index} in {subjects}"));
            ...
            if (!succeeded)
            {
                ...
                if (failedCount >= FailedItemsFastFailThreshold)
                {
                    context.Tracer.WriteLine(member =>
                        $"Aborting strict order comparison of collections after {FailedItemsFastFailThreshold} items failed at {member.Subject}");
    ...
    private void AssertElementGraphEquivalencyWithLooseOrdering<T>(object[] subjects, T[] expectations)
    {
        ...
        foreach (int index in Enumerable.Range(0, expectations.Length))
        {
            ...
            using var _ = context.Tracer.WriteBlock(member =>
                Invariant(
                    $"Finding the best match of {expectation} within all items in {subjects} at {member.Subject}[{index}]"));
            ...
            if (!succeeded)
            {
                ...
                {
                    context.Tracer.WriteLine(member =>
                        $"Fail failing loose order comparison of collection after {FailedItemsFastFailThreshold} items failed at {member.Subject}");
    ...
    private bool LooselyMatchAgainst<T>(IList<object> subjects, T expectation, int expectationIndex)
    {
        ...
        GetTraceMessage getMessage = member =>
            $"Comparing subject at {member.Subject}[{index}] with the expectation at {member.Subject}[{expectationIndex}]";
