...
internal class ExcludeMemberByTypeSelectionRule : IMemberSelectionRule
{
    ...
    public ExcludeMemberByTypeSelectionRule(Type targetType)
    {
        ...
        {
            description = $"Exclude members whose type derives from/is a closed generic type of {targetType}";
        }
        else if (checkAssignability)
        {
            description = $"Exclude members whose type is/derives from {targetType}";
        }
        else
        {
    ...
    private bool ShouldExclude(Type memberType)
    {
        if (targetType.IsGenericTypeDefinition)
        {
            // Check if memberType is a closed generic of the target open generic type, e.g. Nullable<T>
